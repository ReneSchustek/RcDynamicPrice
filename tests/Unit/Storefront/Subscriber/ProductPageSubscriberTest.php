...
final class ProductPageSubscriberTest extends TestCase
{
    ...
    public function testRecordsWidgetShownMetricWhenActive(): void
    {
        $this->configResolver
            ->method('resolveForProduct')
            ->willReturn($this->activeResolved());
        ...
        $this->metrics
            ->expects($this->once())
            ->method('increment')
            ->with(DynamicPriceConstants::METRIC_PRODUCT_PAGE_WIDGET_SHOWN);
    ...
    public function testAddsExtensionWhenResolverReportsActive(): void
    {
        ...
        $this->assertSame('Bitte Länge eingeben', $captured->getHintText());
        $this->assertSame(1000, $captured->getMinLength());
        $this->assertSame(6000, $captured->getMaxLength());
        $this->assertSame(MeterProductHelper::ROUNDING_STEPS, $captured->getRoundingSteps());
    }
